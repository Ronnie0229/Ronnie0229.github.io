...
2026-07-13 Search Console 301 middleware 修复继续：已确认 `functions/_middleware.js` 是本次 Search Console canonical / legacy query 301 修复的核心文件，上一轮将其记录为任务外未跟踪文件是错误状态。本轮将该文件纳入任务范围；文件位置 `functions/_middleware.js` 与 `export async function onRequest(context)` 符合 Cloudflare Pages Functions 根 middleware 规则。已补充 `scripts/test-search-console-middleware.mjs` 覆盖 www、about、分类 query、focus query、正常 URL 和未知参数，并将 middleware 调整为先处理 `/posts/` legacy query，再处理 host canonical redirect。首次线上验证发现 `https://www.ronniecross.com/` 根路径仍返回 200，原因是现有 `_redirects` wildcard 不覆盖根路径；本轮已补充 root 精确 301 规则。当前正在重新构建、提交、push、部署和线上 curl 验证。
