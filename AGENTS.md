...
13. Markdown 文档中的多行命令、路径、日志和文件列表必须使用标准三反引号代码块；代码块开头可以标注 text、shell 或 powershell，结尾单独一行关闭，避免单反引号或转义字符造成格式损坏。
14. 项目文档类修改请求默认直接写入仓库文件；用户说“当前任务文档”时，默认指 `docs/tasks/current.md`。除非用户明确要求“只给我看看，不要改文件”，否则不要只把文档内容留在聊天里。
