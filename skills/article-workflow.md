...
- 中文正文经文引用统一使用受控书卷全名 + `章/节/到` 的 TTS 友好形式，例如 `罗马书 1:15-18` → `罗马书1章15节到18节`；网站导入器必须做 deterministic normalization/gate，frontmatter/SEO/scripture metadata 保留机器格式。
...
- 中文正文中直接引用圣经经文时，默认全部使用《和合本》译文；经文出处与完整范围必须保留，但面向读者的直接经文正文不显示逐节数字节号。连续两节及以上只保留连续经文文字，不在每节前重复 `15 / 16 / 17` 等节号；单节正文也不重复节号。Project Bible / CUVMPS Rv2 仍按逐节记录核验，讲员解释中自然出现的“第5节”等语义性节号不得删除。如需解释英文词语、原文词义或其他译本差异，可在和合本经文之外另加说明，但不得用其他中文译本替代正文经文。
