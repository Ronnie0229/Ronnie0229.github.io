...
3. 任务完成后，必须更新 `STATUS.md` 和 `docs/tasks/current.md`；`current.md` 只保存当前及近期任务，更早的已完成记录按月份归档到 `docs/tasks/archive/`。
4. 任务类型明显切换时，应新开对话。
...
2. `docs/tasks/current.md`：本轮目标、范围和禁止修改范围、已完成事项、修改文件、未完成事项、下一步；默认只保留最近 5 条任务记录以及仍未完成的任务，当文件超过合理规模时继续按月份归档。
