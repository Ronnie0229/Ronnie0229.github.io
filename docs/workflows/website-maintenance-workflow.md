...
```text
/Volumes/DevSSD/RonnieWork/RonnieCross
```
...
Mac 端 NAS 采用最小 scoped symlink 方案，只把 RonnieNAS `tmp` share 下两个收件目录暴露到项目父目录：

```text
/Volumes/DevSSD/RonnieWork/RonnieCross/NAS/分享收件 -> /Volumes/tmp/分享
/Volumes/DevSSD/RonnieWork/RonnieCross/NAS/讲道收件 -> /Volumes/tmp/讲道
```

维护边界：

```text
1. 不把整个 RonnieNAS 软链接进 RonnieCross/NAS。
2. 不链接 share/教会讲道 归档区；需要归档任务时另行授权。
3. 不在 NAS 上运行 Git、Node、Astro、Codex 或文件监听。
4. 内容整理脚本、导出脚本和发布脚本必须等用户明确授权后再运行。
```
...
npm install 成功。
npm run build 成功，213 pages，build 后没有文件变化。
npm audit 当前为 astro/esbuild 相关 1 high / 1 low；暂不运行 npm audit fix 或 npm approve-builds，Astro 7 升级另开任务规划。
```

后续需要路径时，不要把 Windows 的绝对路径硬编码进脚本或文档示例；需要列路径时写明 Windows/Mac 两套示例或使用“当前项目根目录”。
